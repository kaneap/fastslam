...
class Plotter(object):
    ...
    def plot_state(self, sensor_measurements):
        ...
        cov_pose = sum(particle.weight * np.outer(particle.pose - mean_pose, particle.pose - mean_pose) for particle in
                       self.particles) / weight_sum
        ...
        trajectory_x_list = [
            sum(particle.weight * particle.trajectory[i][0] for particle in self.particles) / weight_sum for i in
            range(0, len(self.particles[0].trajectory))]
        trajectory_y_list = [
            sum(particle.weight * particle.trajectory[i][1] for particle in self.particles) / weight_sum for i in
            range(0, len(self.particles[0].trajectory))]
        ...
        for measurement in sensor_measurements:
            ...
            plots.append(
                plt.plot((landmark_x, mean_pose[0]), (landmark_y, mean_pose[1]), 'k', linewidth=1, animated=True)[0])
